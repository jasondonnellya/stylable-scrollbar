...
exports.initStylableScrollbars = void 0;
...
const setupContentContainer = (contentContainer) => {
    const scrollbarId = contentContainer.getAttribute(`${ATTRIBUTE_PREFIX}-scrollable`);
...
const scrollMove = (e) => {
    ...
    const contentContainer = document.querySelector(`[${ATTRIBUTE_PREFIX}-scrollable="${scrollbarId}"]`);
    if (!contentContainer)
        return;
    e.preventDefault();
    e.stopPropagation();
    ...
    if (direction === "horizontal") {
        ...
        let scrollLeft = (contentContainer.scrollWidth - contentContainer.clientWidth) *
            percentage;
        if (scrollLeft < 0)
            scrollLeft = 0;
        ...
        let scrollTop = (contentContainer.scrollHeight - contentContainer.clientHeight) *
            percentage;
        if (scrollTop < 0)
            scrollTop = 0;
...
const initStylableScrollbars = () => {
    const scrollbars = Array.from(document.querySelectorAll(`[${ATTRIBUTE_PREFIX}]`));
    simpleScrollbars = [];
    for (let scrollbarContainer of scrollbars) {
        let scrollbarId = scrollbarContainer.getAttribute(`${ATTRIBUTE_PREFIX}`);
        let contentContainer = document.querySelector(`[${ATTRIBUTE_PREFIX}-scrollable="${scrollbarId}"]`);
        let scrollbarDirection = scrollbarContainer.getAttribute(`direction`);
        simpleScrollbars.push(new SimpleScrollbar({
            scrollbarContainer,
            contentContainer,
            scrollbarId,
            scrollbarDirection,
...
exports.initStylableScrollbars = initStylableScrollbars;
exports.default = exports.initStylableScrollbars;
